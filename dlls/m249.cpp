...
bool CM249::GetItemInfo(ItemInfo* p)
{
	...
	p->iMaxAmmo1 = 180;	 // What's the max ammo quantity for that kind of ammo
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = NULL;
	...
	p->iPosition = 4;  // Which "position" (row) in the HUD this weapon is located (3 = after HL1 crossbow)
...
void CM249::PrimaryAttack()
{
	...
	Vector vecDir = m_pPlayer->FireBulletsPlayer(1, vecSrc, vecAiming, VECTOR_CONE_7DEGREES, 8192, BULLET_PLAYER_MP5,
		1, 7, m_pPlayer->pev, m_pPlayer->random_seed);
...
void CM249::Reload()
{
	...
	DefaultReload(100, FS_RELOAD, 4.3);
}
